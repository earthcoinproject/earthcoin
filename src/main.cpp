...
const string strMessageMagic = "EarthCoin Signed Message:\n";
...
{
    // EarthCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // EarthCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EarthCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EarthCoinMiner : ProcessBlock, block not accepted");
    }
