...
{
    return GetBoolArg("-testnet", false) ? 25678 : 15678;
}
...
Value sendalert(const Array& params, bool fHelp)
{
    ...
    throw runtime_error(
            "sendalert <message> <privatekey> <minver> <maxver> <priority> <id> <time> [cancelupto]\n"
    ...
    int days = params[6].get_int();
    if (params.size() > 7)
        alert.nCancel = params[7].get_int();
    alert.nVersion = PROTOCOL_VERSION;
    alert.nRelayUntil = GetAdjustedTime() + days*24*60*60;
    alert.nExpiration = GetAdjustedTime() + days*24*60*60;
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: earthcoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void StartRPCThreads()
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use earthcoind";
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("%s, you must set a rpcpassword in the configuration file:\n"
              ...
              "rpcuser=earthcoinrpc\n"
              "rpcpassword=%s\n"
              "(you do not need to remember this password)\n"
              "The username and password MUST NOT be the same.\n"
              "If the file does not exist, create it with owner-readable-only file permissions.\n"
              "It is also recommended to set alertnotify so you are notified of problems;\n"
              "for example: alertnotify=echo %%s | mail -s \"EarthCoin Alert\" admin@foo.com\n"),
                strWhatAmI.c_str(),
